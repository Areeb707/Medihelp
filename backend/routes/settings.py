...
async def save_keys(body: SaveKeysRequest):
    ...
    # Fully reset Cognee connection to force reconnect with new keys
    import cognee_client
    cognee_client._connected = False
    cognee_client._last_key  = None
    cognee_client._last_url  = None

    # Validate Cognee key format
    warnings = []
    cognee_key_val = settings.get("cognee_api_key", "")
    if cognee_key_val.startswith("gsk_"):
        warnings.append("Your Cognee API Key looks like a Groq key (starts with gsk_). Cognee keys typically start with 'ck_'. Get yours from https://platform.cognee.ai/apiKeys")
    if cognee_key_val and not settings.get("cognee_base_url"):
        warnings.append("Cognee Base URL is missing. Copy your tenant URL from the Cognee Cloud dashboard.")

    cognee_ok = await cognee_health_check()

    if not cognee_ok and cognee_key_val:
        warnings.append("Cognee health check failed — the API key may be invalid or the Base URL is incorrect.")

    msg = "API keys saved successfully"
    if warnings:
        msg = "API keys saved. WARNING: " + " | ".join(warnings)
