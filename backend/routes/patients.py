...
@router.get("/{patient_id}/evolution")
async def get_patient_evolution(patient_id: str):
    ...
    from llm_client import generate_evolution_explanation
    explanation = await generate_evolution_explanation(diff)
    return explanation
...
async def delete_patient_and_memory(patient_id: str, background_tasks: BackgroundTasks):
    patient = get_patient(patient_id)
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
    
    async def _bg_forget():
        try:
            await cognee_forget(patient_id)
        except Exception as e:
            print(f"[Patients] forget() error: {e}")
            
    background_tasks.add_task(_bg_forget)
