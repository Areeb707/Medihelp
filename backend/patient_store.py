...
def delete_patient(patient_id):
    ...
    if patient_id in data:
        ...
        # Clean up associated alerts for deleted patient
        try:
            from alert_store import ALERTS_PATH, _read as read_alerts, _write as write_alerts
            alerts = read_alerts()
            remaining = [a for a in alerts if a.get("patient_id") != patient_id]
...
def save_structured_diff(patient_id: str, diff: dict):
    data = _read()
    if patient_id in data:
        data[patient_id]["structured_diff"] = diff
        _write(data)


def get_structured_diff(patient_id: str) -> dict:
    data = _read()
    if patient_id in data:
        return data[patient_id].get("structured_diff")
    return None
