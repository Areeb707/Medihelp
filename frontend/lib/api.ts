...
function getDoctorName(): string {
  try {
    const d = localStorage.getItem("medhelp_doctor");
...
export const api = {
  ...
  // Alerts
  getAlerts: (doctor?: string, patientId?: string) => {
    const doc = doctor || getDoctorName();
    let query = doc ? `doctor=${encodeURIComponent(doc)}` : "";
    if (patientId) {
      query += `${query ? "&" : ""}patient_id=${encodeURIComponent(patientId)}`;
    }
    return req<any>("GET", `/alerts${query ? `?${query}` : ""}`);
  },
  getAlertCount: (doctor?: string, patientId?: string) => {
    const doc = doctor || getDoctorName();
    let query = doc ? `doctor=${encodeURIComponent(doc)}` : "";
    if (patientId) {
      query += `${query ? "&" : ""}patient_id=${encodeURIComponent(patientId)}`;
    }
    return req<any>("GET", `/alerts/count${query ? `?${query}` : ""}`);
  },
