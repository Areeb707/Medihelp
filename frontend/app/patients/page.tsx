...
export default function MemoryHubPage() {
  ...
  const loadMindmapBg = async (patientId: string) => {
    try {
      ...
      if (mm) {
        setMindmap(mm);
        cacheSet(`mindmap_${patientId}`, mm);
      }
    } catch {}
  };
        ...
        <div className="page-header">
          <div>
            ...
            <div className="text-[11px] text-ink-muted mt-0.5">
              {displayPatient.name}&apos;s memory hub · {docs.length} documents · {chunks} chunks indexed
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-bg-input border border-line-strong rounded-2xl px-4 py-3">
                <div className="flex items-center gap-3">
                  <input placeholder={`Ask anything about ${displayPatient.name}...`}
                    value={input} onChange={e=>setInput(e.target.value)}
